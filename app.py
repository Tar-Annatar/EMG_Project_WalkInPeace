...
section[data-testid="stSidebar"] * { color: #eafaf0 !important; }
...
div.stButton > button:hover { background-color: var(--g-accent); color: white; }
...
if model is not None and st.session_state.running:
    ...
    while st.session_state.running and loop_iters < max_iters:
        ...
        render_dashboard()
        status_placeholder.markdown(
            f'<span class="badge-live">🟢 LIVE</span> &nbsp; t = {st.session_state.session_t:0.1f}s',
            unsafe_allow_html=True,
        )
